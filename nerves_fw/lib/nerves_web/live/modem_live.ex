defmodule ExNVR.NervesWeb.ModemLive do
  ...
  def render(assigns) do
            ...
            <div class="flex items-center justify-between">
              <div>
                <div class="flex items-center gap-2 mb-1">
                  <div
                    :if={@modem_details.connection == "internet"}
                    class="w-2 h-2 bg-green-500 rounded-full animate-pulse"
                  >
                  </div>
                  <span
                    :if={@modem_details.connection == "internet"}
                    class="text-green-500 font-semibold uppercase text-sm"
                  >
                    CONNECTED
                  </span>

                  <span
                    :if={@modem_details.connection == "disconnected"}
                    class="text-white font-semibold uppercase text-sm"
                  >
                    Disconnected
                  </span>
                </div>
                <p class="text-gray-600 dark:text-gray-400 text-sm">
                  {@modem_details.provider} • {@modem_details.access_tech}
                </p>
              </div>
              <div class="text-right">
                <div class="text-3xl font-bold text-gray-900 dark:text-white">
                  {@modem_details.signal_dbm} <span class="text-sm text-gray-500">dBm</span>
                </div>
          ...
          <div class="bg-white dark:bg-gray-900 rounded-lg p-6 border border-gray-200 dark:border-gray-800">
            <div class="flex justify-between items-center">
              <div>
                ...
                <div class="text-xl font-mono font-semibold text-gray-900 dark:text-gray-100">
                  {@modem_details.ip}/{@modem_details.mask}
                </div>
              </div>
              <div class="text-right">
    ...
    <!-- Hardware -->
        ...
        <div :if={@messages != []}>
          <table class="min-w-full mx-auto border border-gray-200 bg-white shadow-sm dark:bg-gray-900/50 dark:border-gray-800 my-5 ">
            <!-- Table Header -->
            <thead class="bg-gray-50 border-b border-gray-200 dark:bg-gray-900/20 dark:border-gray-800">
              <tr>
                ...
                <th class="px-6 py-3 text-right text-sm font-medium text-gray-600 dark:text-gray-400">
                  Status
                </th>
              </tr>
            </thead>
  ...
  def mount(_params, _session, socket) do
    Phoenix.PubSub.subscribe(ExNVR.Nerves.PubSub, "messages")

    {:ok, assign(socket, modem_details: get_modem_data(), messages: [])}
  end
