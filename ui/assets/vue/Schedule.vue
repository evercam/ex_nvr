...
function getRandomSegment() {
  ...
  const endHour = Math.floor(Math.random() * (24 - startHour)) + startHour + 1
...
function generateRandomSchedule(days: DayCode[]): DaySchedule[] {
  return days.map((day) => ({
    day,
    segments: Array.from(
...
export default defineComponent({
  ...
  props: {
    days: {
      ...
      default: () => generateRandomSchedule([0, 1, 2, 3, 4, 5, 6]),
    },
  },
  data() {
    return {
