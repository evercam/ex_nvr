defmodule ExNVR.Pipeline.Output.ObjectDetection.Decoder do
  ...
  def handle_init(_ctx, _options) do
    {[], %{decoder: nil, source_width: nil, source_height: nil}}
  end
  ...
  def handle_stream_format(:input, stream_format, %{pads: %{input: %{stream_format: nil}}}, state) do
    ...
    {[
     ...
     %{state | decoder: decoder, source_width: stream_format.width, source_height: stream_format.height}}
  end
  ...
  def handle_buffer(:input, buffer, _ctx, state) do
    buffers =
      ...
      |> Enum.map(fn frame ->
        metadata =
          buffer.metadata
          |> Map.put(:frame_width, state.source_width || frame.width)
          |> Map.put(:frame_height, state.source_height || frame.height)

        %Membrane.Buffer{pts: frame.pts, payload: frame.data, metadata: metadata}
      end)
