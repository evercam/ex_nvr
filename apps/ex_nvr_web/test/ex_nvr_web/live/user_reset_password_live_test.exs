defmodule ExNVRWeb.UserResetPasswordLiveTest do
    ...
    test "resets password once", %{conn: conn, token: token, user: user} do
      ...
      {:ok, conn} =
        ...
        |> form("#reset_password_form",
          user: %{
            "password" => "?ew Valid password",
            "password_confirmation" => "?ew Valid password"
          }
        )
        |> render_submit()
      ...
      assert Accounts.get_user_by_email_and_password(user.email, "?ew Valid password")
    end
